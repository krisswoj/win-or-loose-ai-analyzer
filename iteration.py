        ...
        if filename.endswith('.txt'):
            with open(tetris_results_path + '/' + filename, encoding='utf-8', errors='replace') as f:
                ...
                for i, result in enumerate(tetris_grouped_results):
                    ...
                    sfdgh = str(read_tetris_results(end_date))

                    with open('tetris_results_new/' + str(tempfilename[0:3]) + '_' + str(tempfilename[4:6]) + '_' + sfdgh + '__' + format_to_print(start_date) + '__' + str(time_diff) + 's.txt', "w") as g:
                        g.writelines(temp)
